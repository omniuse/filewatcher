...
public class AVMain implements FileChangeListener {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length == 0) {
			...
			new AVMain().run(new String[] { firstParameterDefault, secondParameterDefault });
		}
		// run with arguments instead of default parameters
		else {
			new AVMain().run(args);
		}
	}
	...
	private void run(String[] args) throws Exception {
		...
		ParameterHighLevel parameter = new ParameterHighLevel(args);
		...
		if (parameter.isValid() && parameter.isSecondParameterC()) {
